...
func InitGenesis(ctx sdk.Context, keeper keeper.Keeper, data *types.GenesisState) []abci.ValidatorUpdate {
	for _, record := range data.Certificates {
		id, _ := types.ToCertID(record.ID)

		err := keeper.CreateCertificate(ctx, id, record.Certificate.Cert, record.Certificate.Pubkey)
		if err != nil {
			ctx.Logger().Error("error init genesis cert: %s", err)
		}
	}
