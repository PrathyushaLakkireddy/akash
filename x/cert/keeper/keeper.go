...
func (k Keeper) CreateCertificate(ctx sdk.Context, id types.CertID, crt []byte, pubkey []byte) error {
	...
	iter := sdk.KVStorePrefixIterator(store, certificatePrefix(id.Owner))
	defer func() {
		_ = iter.Close()
	}()
